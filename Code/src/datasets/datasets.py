...
def build_movie_n_reviews_to_review_dataset(n=10):
...
def build_movie_n_review_n_1_rating_to_rating_dataset(n):
    users_output_filtered_df = users_output_df[['movieID', 'userID', 'target', 'rating']]
    users_output_filtered_df.dropna(inplace=True)
...
movies_input_df = None
users_output_df = None
...
if CODE == 1:
    # création du dataset movie_n_reviews_to_review
    n = int(sys.argv[2])
    df = build_movie_n_reviews_to_review_dataset(n)
    df.to_csv(BASE_DIR + 'appdata/rotten/movie_n_reviews_to_review.csv', index=True)

elif CODE == 2:
    # création du dataset movie_review_to_rating
    df = build_movie_review_to_rating_dataset()
    df.to_csv(BASE_DIR + 'appdata/rotten/movie_review_to_rating.csv', index=True)

elif CODE == 3:
    # création du dataset review_to_rating
    df = build_review_to_rating_dataset()
    df.to_csv(BASE_DIR + 'appdata/rotten/review_to_rating.csv', index=True)


elif CODE == 4:
    # création du dataset movie_n_review_n_1_rating_to_rating
    pass
